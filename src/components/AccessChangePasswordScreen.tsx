...
type AccessChangePasswordScreenProps = {
  email: string;
  fullName: string;
...
export default function AccessChangePasswordScreen({
  email,
  fullName,
  currentPassword,
  title = "Define tu clave",
  description = "Ingresaste con una clave temporal. Antes de continuar, crea tu clave definitiva.",
  onCompleted,
  onCancel,
}: AccessChangePasswordScreenProps) {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/accesos/change-password", {
        method: "POST",
    ...
    <main className="flex min-h-[100dvh] items-center justify-center bg-[#eef3f9] px-4 py-6 text-[#0f2747] sm:px-6 sm:py-10 lg:px-10">
      ...
      >
        <div className="mb-7 border-b border-[#c5d8eb] pb-6">
          ...
          {fullName ? (
            <p className="mt-3 text-sm font-medium text-[#173b68]">{fullName}</p>
          ) : null}
